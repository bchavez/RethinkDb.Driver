




//AUTOGENERATED, DO NOTMODIFY.
//Do not edit this file directly.

#pragma warning disable 1591
// ReSharper disable CheckNamespace

using System;
using System.Linq;
using System.Collections;
using System.Collections.Generic;
using RethinkDb.Driver.Model;
using RethinkDb.Driver.Ast;
using NUnit.Framework;
using RethinkDb.Driver.Tests;

namespace RethinkDb.Driver.Test.Generated {
    [TestFixture]
    public class GeoConstructors : GeneratedTest {



        [Test]
        public void YamlTest(){

             TestCounter++;
             
             {
                 //JavaQuery, geo/constructors.yaml, #1
                 //ExpectedOriginal: ({'$reql_type$':'GEOMETRY', 'coordinates':[0, 0], 'type':'Point'})
                 var expected = r.hashMap("$reql_type$", "GEOMETRY").with("coordinates", r.array(0.0, 0.0)).with("type", "Point");
                 
                 //Original: r.point(0, 0)
                 var obtained = runOrCatch( r.point(0.0, 0.0) ,
                                            new OptArgs()
                    );
                 assertEquals(expected, obtained);
             }
             
             TestCounter++;
             
             {
                 //JavaQuery, geo/constructors.yaml, #2
                 //ExpectedOriginal: ({'$reql_type$':'GEOMETRY', 'coordinates':[0, -90], 'type':'Point'})
                 var expected = r.hashMap("$reql_type$", "GEOMETRY").with("coordinates", r.array(0.0, -90.0)).with("type", "Point");
                 
                 //Original: r.point(0, -90)
                 var obtained = runOrCatch( r.point(0.0, -90.0) ,
                                            new OptArgs()
                    );
                 assertEquals(expected, obtained);
             }
             
             TestCounter++;
             
             {
                 //JavaQuery, geo/constructors.yaml, #3
                 //ExpectedOriginal: ({'$reql_type$':'GEOMETRY', 'coordinates':[0, 90], 'type':'Point'})
                 var expected = r.hashMap("$reql_type$", "GEOMETRY").with("coordinates", r.array(0.0, 90.0)).with("type", "Point");
                 
                 //Original: r.point(0, 90)
                 var obtained = runOrCatch( r.point(0.0, 90.0) ,
                                            new OptArgs()
                    );
                 assertEquals(expected, obtained);
             }
             
             TestCounter++;
             
             {
                 //JavaQuery, geo/constructors.yaml, #4
                 //ExpectedOriginal: ({'$reql_type$':'GEOMETRY', 'coordinates':[-180, 0], 'type':'Point'})
                 var expected = r.hashMap("$reql_type$", "GEOMETRY").with("coordinates", r.array(-180.0, 0.0)).with("type", "Point");
                 
                 //Original: r.point(-180, 0)
                 var obtained = runOrCatch( r.point(-180.0, 0.0) ,
                                            new OptArgs()
                    );
                 assertEquals(expected, obtained);
             }
             
             TestCounter++;
             
             {
                 //JavaQuery, geo/constructors.yaml, #5
                 //ExpectedOriginal: ({'$reql_type$':'GEOMETRY', 'coordinates':[180, 0], 'type':'Point'})
                 var expected = r.hashMap("$reql_type$", "GEOMETRY").with("coordinates", r.array(180.0, 0.0)).with("type", "Point");
                 
                 //Original: r.point(180, 0)
                 var obtained = runOrCatch( r.point(180.0, 0.0) ,
                                            new OptArgs()
                    );
                 assertEquals(expected, obtained);
             }
             
             TestCounter++;
             
             {
                 //JavaQuery, geo/constructors.yaml, #6
                 //ExpectedOriginal: err('ReqlQueryLogicError', 'Latitude must be between -90 and 90.  Got -91.', [0])
                 var expected = err("ReqlQueryLogicError", "Latitude must be between -90 and 90.  Got -91.", r.array(0.0));
                 
                 //Original: r.point(0, -91)
                 var obtained = runOrCatch( r.point(0.0, -91.0) ,
                                            new OptArgs()
                    );
                 assertEquals(expected, obtained);
             }
             
             TestCounter++;
             
             {
                 //JavaQuery, geo/constructors.yaml, #7
                 //ExpectedOriginal: err('ReqlQueryLogicError', 'Latitude must be between -90 and 90.  Got 91.', [0])
                 var expected = err("ReqlQueryLogicError", "Latitude must be between -90 and 90.  Got 91.", r.array(0.0));
                 
                 //Original: r.point(0, 91)
                 var obtained = runOrCatch( r.point(0.0, 91.0) ,
                                            new OptArgs()
                    );
                 assertEquals(expected, obtained);
             }
             
             TestCounter++;
             
             {
                 //JavaQuery, geo/constructors.yaml, #8
                 //ExpectedOriginal: err('ReqlQueryLogicError', 'Longitude must be between -180 and 180.  Got -181.', [0])
                 var expected = err("ReqlQueryLogicError", "Longitude must be between -180 and 180.  Got -181.", r.array(0.0));
                 
                 //Original: r.point(-181, 0)
                 var obtained = runOrCatch( r.point(-181.0, 0.0) ,
                                            new OptArgs()
                    );
                 assertEquals(expected, obtained);
             }
             
             TestCounter++;
             
             {
                 //JavaQuery, geo/constructors.yaml, #9
                 //ExpectedOriginal: err('ReqlQueryLogicError', 'Longitude must be between -180 and 180.  Got 181.', [0])
                 var expected = err("ReqlQueryLogicError", "Longitude must be between -180 and 180.  Got 181.", r.array(0.0));
                 
                 //Original: r.point(181, 0)
                 var obtained = runOrCatch( r.point(181.0, 0.0) ,
                                            new OptArgs()
                    );
                 assertEquals(expected, obtained);
             }
             
             TestCounter++;
             
             {
                 //JavaQuery, geo/constructors.yaml, #12
                 //ExpectedOriginal: err('ReqlQueryLogicError', 'Invalid LineString.  Are there antipodal or duplicate vertices?', [0])
                 var expected = err("ReqlQueryLogicError", "Invalid LineString.  Are there antipodal or duplicate vertices?", r.array(0.0));
                 
                 //Original: r.line([0,0], [0,0])
                 var obtained = runOrCatch( r.line(r.array(0.0, 0.0), r.array(0.0, 0.0)) ,
                                            new OptArgs()
                    );
                 assertEquals(expected, obtained);
             }
             
             TestCounter++;
             
             {
                 //JavaQuery, geo/constructors.yaml, #13
                 //ExpectedOriginal: ({'$reql_type$':'GEOMETRY', 'coordinates':[[0,0], [0,1]], 'type':'LineString'})
                 var expected = r.hashMap("$reql_type$", "GEOMETRY").with("coordinates", r.array(r.array(0.0, 0.0), r.array(0.0, 1.0))).with("type", "LineString");
                 
                 //Original: r.line([0,0], [0,1])
                 var obtained = runOrCatch( r.line(r.array(0.0, 0.0), r.array(0.0, 1.0)) ,
                                            new OptArgs()
                    );
                 assertEquals(expected, obtained);
             }
             
             TestCounter++;
             
             {
                 //JavaQuery, geo/constructors.yaml, #14
                 //ExpectedOriginal: err('ReqlQueryLogicError', 'Expected point coordinate pair.  Got 1 element array instead of a 2 element one.', [0])
                 var expected = err("ReqlQueryLogicError", "Expected point coordinate pair.  Got 1 element array instead of a 2 element one.", r.array(0.0));
                 
                 //Original: r.line([0,0], [1])
                 var obtained = runOrCatch( r.line(r.array(0.0, 0.0), r.array(1.0)) ,
                                            new OptArgs()
                    );
                 assertEquals(expected, obtained);
             }
             
             TestCounter++;
             
             {
                 //JavaQuery, geo/constructors.yaml, #15
                 //ExpectedOriginal: err('ReqlQueryLogicError', 'Expected point coordinate pair.  Got 3 element array instead of a 2 element one.', [0])
                 var expected = err("ReqlQueryLogicError", "Expected point coordinate pair.  Got 3 element array instead of a 2 element one.", r.array(0.0));
                 
                 //Original: r.line([0,0], [1,0,0])
                 var obtained = runOrCatch( r.line(r.array(0.0, 0.0), r.array(1.0, 0.0, 0.0)) ,
                                            new OptArgs()
                    );
                 assertEquals(expected, obtained);
             }
             
             TestCounter++;
             
             {
                 //JavaQuery, geo/constructors.yaml, #16
                 //ExpectedOriginal: ({'$reql_type$':'GEOMETRY', 'coordinates':[[0,0], [0,1], [0,0]], 'type':'LineString'})
                 var expected = r.hashMap("$reql_type$", "GEOMETRY").with("coordinates", r.array(r.array(0.0, 0.0), r.array(0.0, 1.0), r.array(0.0, 0.0))).with("type", "LineString");
                 
                 //Original: r.line([0,0], [0,1], [0,0])
                 var obtained = runOrCatch( r.line(r.array(0.0, 0.0), r.array(0.0, 1.0), r.array(0.0, 0.0)) ,
                                            new OptArgs()
                    );
                 assertEquals(expected, obtained);
             }
             
             TestCounter++;
             
             {
                 //JavaQuery, geo/constructors.yaml, #17
                 //ExpectedOriginal: ({'$reql_type$':'GEOMETRY', 'coordinates':[[0,0], [0,1], [0,0]], 'type':'LineString'})
                 var expected = r.hashMap("$reql_type$", "GEOMETRY").with("coordinates", r.array(r.array(0.0, 0.0), r.array(0.0, 1.0), r.array(0.0, 0.0))).with("type", "LineString");
                 
                 //Original: r.line(r.point(0,0), r.point(0,1), r.point(0,0))
                 var obtained = runOrCatch( r.line(r.point(0.0, 0.0), r.point(0.0, 1.0), r.point(0.0, 0.0)) ,
                                            new OptArgs()
                    );
                 assertEquals(expected, obtained);
             }
             
             TestCounter++;
             
             {
                 //JavaQuery, geo/constructors.yaml, #18
                 //ExpectedOriginal: err('ReqlQueryLogicError', 'Expected geometry of type `Point` but found `LineString`.', [0])
                 var expected = err("ReqlQueryLogicError", "Expected geometry of type `Point` but found `LineString`.", r.array(0.0));
                 
                 //Original: r.line(r.point(0,0), r.point(1,0), r.line([0,0], [1,0]))
                 var obtained = runOrCatch( r.line(r.point(0.0, 0.0), r.point(1.0, 0.0), r.line(r.array(0.0, 0.0), r.array(1.0, 0.0))) ,
                                            new OptArgs()
                    );
                 assertEquals(expected, obtained);
             }
             
             TestCounter++;
             
             {
                 //JavaQuery, geo/constructors.yaml, #22
                 //ExpectedOriginal: err('ReqlQueryLogicError', 'Invalid LinearRing.  Are there antipodal or duplicate vertices? Is it self-intersecting?', [0])
                 var expected = err("ReqlQueryLogicError", "Invalid LinearRing.  Are there antipodal or duplicate vertices? Is it self-intersecting?", r.array(0.0));
                 
                 //Original: r.polygon([0,0], [0,0], [0,0], [0,0])
                 var obtained = runOrCatch( r.polygon(r.array(0.0, 0.0), r.array(0.0, 0.0), r.array(0.0, 0.0), r.array(0.0, 0.0)) ,
                                            new OptArgs()
                    );
                 assertEquals(expected, obtained);
             }
             
             TestCounter++;
             
             {
                 //JavaQuery, geo/constructors.yaml, #23
                 //ExpectedOriginal: ({'$reql_type$':'GEOMETRY', 'coordinates':[[[0,0], [0,1], [1,0], [0,0]]], 'type':'Polygon'})
                 var expected = r.hashMap("$reql_type$", "GEOMETRY").with("coordinates", r.array(r.array(r.array(0.0, 0.0), r.array(0.0, 1.0), r.array(1.0, 0.0), r.array(0.0, 0.0)))).with("type", "Polygon");
                 
                 //Original: r.polygon([0,0], [0,1], [1,0])
                 var obtained = runOrCatch( r.polygon(r.array(0.0, 0.0), r.array(0.0, 1.0), r.array(1.0, 0.0)) ,
                                            new OptArgs()
                    );
                 assertEquals(expected, obtained);
             }
             
             TestCounter++;
             
             {
                 //JavaQuery, geo/constructors.yaml, #24
                 //ExpectedOriginal: ({'$reql_type$':'GEOMETRY', 'coordinates':[[[0,0], [0,1], [1,0], [0,0]]], 'type':'Polygon'})
                 var expected = r.hashMap("$reql_type$", "GEOMETRY").with("coordinates", r.array(r.array(r.array(0.0, 0.0), r.array(0.0, 1.0), r.array(1.0, 0.0), r.array(0.0, 0.0)))).with("type", "Polygon");
                 
                 //Original: r.polygon([0,0], [0,1], [1,0], [0,0])
                 var obtained = runOrCatch( r.polygon(r.array(0.0, 0.0), r.array(0.0, 1.0), r.array(1.0, 0.0), r.array(0.0, 0.0)) ,
                                            new OptArgs()
                    );
                 assertEquals(expected, obtained);
             }
             
             TestCounter++;
             
             {
                 //JavaQuery, geo/constructors.yaml, #25
                 //ExpectedOriginal: err('ReqlQueryLogicError', 'Invalid LinearRing.  Are there antipodal or duplicate vertices? Is it self-intersecting?', [0])
                 var expected = err("ReqlQueryLogicError", "Invalid LinearRing.  Are there antipodal or duplicate vertices? Is it self-intersecting?", r.array(0.0));
                 
                 //Original: r.polygon([0,0], [0,1], [1,0], [-1,0.5])
                 var obtained = runOrCatch( r.polygon(r.array(0.0, 0.0), r.array(0.0, 1.0), r.array(1.0, 0.0), r.array(-1.0, 0.5)) ,
                                            new OptArgs()
                    );
                 assertEquals(expected, obtained);
             }
             
             TestCounter++;
             
             {
                 //JavaQuery, geo/constructors.yaml, #26
                 //ExpectedOriginal: err('ReqlQueryLogicError', 'Expected point coordinate pair.  Got 1 element array instead of a 2 element one.', [0])
                 var expected = err("ReqlQueryLogicError", "Expected point coordinate pair.  Got 1 element array instead of a 2 element one.", r.array(0.0));
                 
                 //Original: r.polygon([0,0], [0,1], [0])
                 var obtained = runOrCatch( r.polygon(r.array(0.0, 0.0), r.array(0.0, 1.0), r.array(0.0)) ,
                                            new OptArgs()
                    );
                 assertEquals(expected, obtained);
             }
             
             TestCounter++;
             
             {
                 //JavaQuery, geo/constructors.yaml, #27
                 //ExpectedOriginal: err('ReqlQueryLogicError', 'Expected point coordinate pair.  Got 3 element array instead of a 2 element one.', [0])
                 var expected = err("ReqlQueryLogicError", "Expected point coordinate pair.  Got 3 element array instead of a 2 element one.", r.array(0.0));
                 
                 //Original: r.polygon([0,0], [0,1], [0,1,0])
                 var obtained = runOrCatch( r.polygon(r.array(0.0, 0.0), r.array(0.0, 1.0), r.array(0.0, 1.0, 0.0)) ,
                                            new OptArgs()
                    );
                 assertEquals(expected, obtained);
             }
             
             TestCounter++;
             
             {
                 //JavaQuery, geo/constructors.yaml, #28
                 //ExpectedOriginal: err('ReqlQueryLogicError', 'Expected geometry of type `Point` but found `LineString`.', [0])
                 var expected = err("ReqlQueryLogicError", "Expected geometry of type `Point` but found `LineString`.", r.array(0.0));
                 
                 //Original: r.polygon(r.point(0,0), r.point(0,1), r.line([0,0], [0,1]))
                 var obtained = runOrCatch( r.polygon(r.point(0.0, 0.0), r.point(0.0, 1.0), r.line(r.array(0.0, 0.0), r.array(0.0, 1.0))) ,
                                            new OptArgs()
                    );
                 assertEquals(expected, obtained);
             }
             

        }
    }
}
