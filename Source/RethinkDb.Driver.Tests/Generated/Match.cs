    ...
    public class Match : GeneratedTest {
        ...
        public void YamlTest(){
             ...
             
             {
                 ...
                 var expected = r.hashMap("str", "bcde").with("groups", r.array(null, r.hashMap("start", 2.0).with("str", "cde").with("end", 5.0))).with("start", 1.0).with("end", 5.0);
             ...
             
             {
                 ...
                 var expected = r.array("aca", "ada");
                 
                 //Original: r.expr(["aba", "aca", "ada", "aea"]).filter(lambda row:row.match("a(.)a")['groups'][0]['str'].match("[cd]"))
                 var obtained = runOrCatch( r.expr(r.array("aba", "aca", "ada", "aea")).filter(row => row.match("a(.)a").bracket("groups").bracket(0.0).bracket("str").match("[cd]")) ,
                                            new OptArgs()
                    );
                 assertEquals(expected, obtained);
             }
             ...
             
             {
                 ...
                 var expected = r.hashMap("deleted", 0.0).with("replaced", 0.0).with("unchanged", 0.0).with("errors", 0.0).with("skipped", 0.0).with("inserted", 3.0);
                 
                 //Original: tbl.insert([{'id':0,'a':'abc'},{'id':1,'a':'ab'},{'id':2,'a':'bc'}])
                 var obtained = runOrCatch( tbl.insert(r.array(r.hashMap("id", 0.0).with("a", "abc"), r.hashMap("id", 1.0).with("a", "ab"), r.hashMap("id", 2.0).with("a", "bc"))) ,
                                            new OptArgs()
                    );
                 assertEquals(expected, obtained);
             }
             ...
             
             {
                 ...
                 var expected = r.array(r.hashMap("id", 0.0).with("a", "abc"), r.hashMap("id", 1.0).with("a", "ab"), r.hashMap("id", 2.0).with("a", "bc"));
             ...
             
             {
                 ...
                 var expected = r.array(r.hashMap("id", 0.0).with("a", "abc"), r.hashMap("id", 1.0).with("a", "ab"));
             ...
             
             {
                 ...
                 var expected = r.array(r.hashMap("id", 1.0).with("a", "ab"));
             ...
             
             {
                 ...
                 var expected = err("ReqlQueryLogicError", "Error in regexp `ab\\9` (portion `\\9`): invalid escape sequence: \\9", r.array());
